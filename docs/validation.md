...
```
warning option-unknown 10:5 markdag のキー「detail」は使えません (edgeHighlight, groupHighlight, details, legend ほか)
    もしかして「details」
...
- `info not-extracted` means the frontmatter has none of `relations`, `groups`, `markdag`, so tags, `$id`, details and milestones were not extracted. It also appears after a `yaml-syntax` error, because a frontmatter that fails to parse is ignored as a whole.
...
| `option-misplaced` | warning | Shape and type | A key written at the wrong level (for example `branches` outside `markdag`). It is ignored |
...
| `relation-unknown-key` | warning | Shape and type | A key under `relations` other than `fork`, `join`, `chain`, `depends` |
...
| `group-invalid` | warning | Shape and type | Wrong type or value under `groups` (unquoted color, `boundary: yes`), or `(X)` in `members` |
...
| `not-extracted` | info | Body | The frontmatter has none of `relations`, `groups`, `markdag`. The document is shown as a plain markmap |
| `not-supported` | warning | Graph | `(X)` is not supported yet and is treated as `X` |

The reference errors are reported as warnings when they come from `groups.*.members` or `markdag.branches` instead of `relations`.
